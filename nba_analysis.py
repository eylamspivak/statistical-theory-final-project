"""
NBA Career Longevity - Statistical Analysis (v2 - reviewed & corrected)
עילם ספיבק | אסף בוזגלו
תיאוריה סטטיסטית, בר-אילן 2026

CHANGES FROM v1 (based on methodological review):
  - Incident-cohort filtering: drops players whose first *recorded* season
    is the dataset's very first season (1996-97), since for those we cannot
    tell if that was their true rookie season or if they were already
    veterans (left-truncation / prevalent-cohort bias).
  - career_length now reports both span (max-min+1) and nunique(), flags
    how many players differ, and uses span consistently with the paper.
  - Fixed groupby().first() row-mixing bug -> uses .head(1) after sorting.
  - t-tests use Welch's correction (equal_var=False), matching the SE
    formula already used for confidence intervals.
  - p-values computed via .sf() to avoid underflow to exactly 0.
  - Goodness-of-fit: added discrete chi-square GOF (appropriate for
    integer-valued career_length) and a Geometric-distribution MLE fit.
  - Added Levene's test (variance homogeneity) and Kruskal-Wallis
    (non-parametric ANOVA counterpart) to H2.
  - Added post-hoc power analysis + power curve.
  - Added nested regression model comparison (M1 performance-only /
    M2 draft-only / M3 both) with an explicit GLRT framing, so the
    "draft round is the strongest factor" claim is actually tested
    against performance in one joint model.
  - Wald SPRT now runs 1000 random-seed repetitions and reports the
    distribution of decisions and stopping times, instead of one run.
"""
...
import warnings
# NOTE: we deliberately do NOT blanket-suppress warnings (v1 did, which
# hid legitimate ties/precision warnings from KS and other tests).
...
def load_data(path="all_seasons.csv", first_dataset_year=1996):
    ...
    *recorded* row is 1996-97 could actually be a veteran who debuted
    ...
    # ---- Two career-length definitions; report divergence ----------
    span = (df.groupby("player_name")["year"]
              .agg(lambda x: x.max() - x.min() + 1))
    nuniq = df.groupby("player_name")["year"].nunique()
    diff_players = (span != nuniq).sum()
    print(f"  ℹ️  career-length definitions: span vs. distinct-season-count "
          f"differ for {diff_players} players (players with a gap season). "
...
def goodness_of_fit(data):
    ...
    d_e, p_e = kstest(cl, "expon", args=(1, cl.mean() - 1))  # FIX: loc=1 (min of data)
    ...
    # ---- Discrete chi-square GOF vs. Normal (binned) -----------------
    # More appropriate than KS here since career_length is a small
    # integer (1-25) with many ties, violating KS's continuity assumption.
    bins = [0, 1, 2, 3, 4, 5, 7, 9, 12, 100]
    obs, edges = np.histogram(cl, bins=bins)
    cdf_vals = stats.norm.cdf(edges, mu, sigma)
    exp_p = np.diff(cdf_vals)
    exp = exp_p * n
    chi2_stat = np.sum((obs - exp)**2 / exp)
    dof = len(obs) - 1 - 2  # -2 for estimated mu, sigma
    p_chi2 = stats.chi2.sf(chi2_stat, dof)
    print(f"\n  Discrete χ² GOF vs Normal (binned, appropriate for integer "
          f"data):")
    ...
    # Chi-square GOF for the geometric fit, same bins as above
    exp_p_geom = np.array([
        (1 - p_hat)**(edges[i]) - (1 - p_hat)**(edges[i+1])
        if edges[i+1] < 100 else (1 - p_hat)**(edges[i])
        for i in range(len(edges) - 1)
    ])
    exp_p_geom = exp_p_geom / exp_p_geom.sum()  # renormalize for open-ended last bin
    exp_geom = exp_p_geom * n
    ...
    print(f"    χ² GOF: χ²={chi2_geom:.3f}, df={dof_geom}, {fmt_p(p_geom)}  "
          f"{'✅ geometric fits' if p_geom>0.05 else '❌ geometric rejected — hazard is likely not constant (see Discussion)'}")
...
def hypothesis_ppg(data):
    ...
    # FIX: Welch's t-test (equal_var=False), matching the Welch SE
    # formula already used below for the confidence interval -- v1
    # used Student's pooled-variance test by default, which didn't
    # match the CI formula or the Methods section's stated formula.
    ...
    d_range = np.array([0.1, 0.2, 0.3, 0.5, 0.71])
...
def hypothesis_draft_round(data):
    ...
    # ---- ADD: Kruskal-Wallis (non-parametric ANOVA counterpart) -----
    # v1 paired every parametric test with a non-parametric one EXCEPT
    # ANOVA -- this fills that gap, and is more trustworthy given the
    # Levene result above and the right-skew established earlier.
...
def regression(data):
    print("\n" + "═"*55)
    print("  MULTIPLE LINEAR REGRESSION  (OLS, performance + physio only)")
    print("═"*55)
    ...
    ax.set_title("Regression Coefficients ± 95% CI\n(performance + physio only — see Fig. M1/M2/M3 for draft-round comparison)",
...
def nested_model_comparison(data):
    print("\n" + "═"*55)
    print("  NESTED MODEL COMPARISON: Performance vs. Draft Round")
    print("  (M1: performance only | M2: draft only | M3: both)")
    print("═"*55)
    ...
    print(f"\n  M1 (performance only, k={len(perf_feats)}):  R²={m1['r2']:.4f}")
    ...
    print(f"    {'❌ Reject H0 — draft group adds significant explanatory power beyond performance' if p_F1 < 0.05 else '✅ Fail to reject'}")

    q2 = m3['k'] - m2['k']
    F2 = ((m2['ss_res'] - m3['ss_res']) / q2) / (m3['ss_res'] / (n - m3['k'] - 1))
    p_F2 = stats.f.sf(F2, q2, n - m3['k'] - 1)
    print(f"\n  Nested F-test (M2 -> M3, does performance add power beyond draft group?):")
    print(f"    F({q2}, {n-m3['k']-1}) = {F2:.3f}, {fmt_p(p_F2)}")
    print(f"    {'❌ Reject H0 — performance adds significant explanatory power beyond draft group' if p_F2 < 0.05 else '✅ Fail to reject'}")

    print(f"\n  >> Interpretation: both directions {'are' if (p_F1<0.05 and p_F2<0.05) else 'are NOT both'} "
          f"significant, meaning performance and draft group each carry "
          f"independent explanatory power not subsumed by the other.")

    # Bar chart comparing the three models' R²
    fig, ax = plt.subplots(figsize=(7, 5))
    model_names = ["M1: Performance\nonly", "M2: Draft Round\nonly", "M3: Both\ncombined"]
...
def regression_interaction(data):
    ...
    if p_round2 < 0.05 and p_undrafted >= 0.05:
        ...
        print(f"  ⚠️  NOTE: the 'performance compensates for low draft position' "
              f"story is only supported for Round-2+ picks, NOT for undrafted "
              f"players. Only {n_high_ppg_undrafted} undrafted players have "
              f"PPG_z > 1 (high scoring) in this sample — the Undrafted group "
              f"may simply be too small/right-truncated in scoring range to "
              f"detect an effect, rather than performance genuinely not "
              f"mattering for them. This should be stated explicitly, not "
              f"glossed over, in the paper.")
...
def classification_model(data):
    ...
    features = [c for c in ["PPG", "RPG", "APG", "net_rtg", "usg",
                             "height_cm", "weight_kg", "draft_age"]
                if c in data.columns]
    sub = data[features + ["career_length"]].dropna().copy()
    ...
    print(f"  Majority-class baseline accuracy: {majority_class_pct:.1%}  "
          f"(FIX: this — not 50% — is the correct comparison baseline)")
...
def cross_validation(data):
    ...
    ax.set_xlabel("Fold")
    ax.set_ylabel("R²")
    ax.set_title("5-Fold Cross-Validation — Regression Stability",
                 color=NAVY, fontweight="bold")
...
#
#    FIX #1 (formula bug): the log-likelihood-ratio increment for
#    comparing N(mu1,sigma) vs N(mu0,sigma) is
#        logLR(x) = (mu1-mu0)/sigma^2 * x  -  (mu1^2-mu0^2)/(2*sigma^2)
#    v1's code computed a DIFFERENT expression that included a spurious
#    extra term -mu0*(mu1-mu0)/sigma^2 per observation, biasing every
#    single step toward "Accept H0" regardless of the data. This alone
#    -- not just small-n noise -- explains why v1 stopped at n=6 and
#    accepted H0 even though the subgroup's true mean (6.72) was closer
#    to H1 (6.60) than to H0 (5.29).
#
#    FIX #2 (single seed -> 1000 seeds): a single run's stopping point
#    is not representative; we now report the full distribution of
#    decisions and stopping times, plus the empirical Average Sample
#    Number (ASN) compared to Wald's theoretical approximation.
# ════════════════════════════════════════════════════════════════════
...
def wald_sequential_test(data, alpha=0.05, beta=0.20, n_sims=1000):
    ...
    for seed in range(n_sims):
        ...
        if stop_n is not None:
            stops.append(stop_n)
        if seed == 0:
            example_path = (path, stop_n, decision)
    ...
    ASN_H1_theory = ((1 - beta) * B + beta * A) / (-EZ_H1) if EZ_H1 != 0 else np.nan
    # (sign convention: this gives an approximate expected sample size under H1)
...
if __name__ == "__main__":
    print("\n🏀  NBA Career Longevity – Full Statistical Analysis (v2)\n")
    ...
    print("\n✅  Done! Figures saved as fig1–fig17 PNG files.")
