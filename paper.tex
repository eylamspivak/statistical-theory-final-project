...
\author{
  ...
  \small \texttt{[REPLACE: real emails]} \quad \texttt{[REPLACE: ID numbers]}
}
\date{}
...
\twocolumn[
  ...
  \parbox{0.92\textwidth}{\small
    ...
    ($r=-0.294$, $p<0.0001$). Critically, when performance and draft
    round were entered into a single nested-model comparison, both
    contributed statistically independent explanatory power (nested
    $F$-tests / GLRT, both $p<0.0001$): $R^2$ rose from 0.203
    (performance only) and 0.184 (draft round only) to 0.251 combined
    --- directly confirming, on a modern cohort, the sunk-cost pattern
    documented by \citet{staw1995sunk}. A logistic classifier
    distinguishing short from long careers reached 63.8\% accuracy
    against a 50.7\% majority-class baseline (AUC$=0.693$). A
    corrected, 1000-simulation Wald sequential test supported the PPG
    effect in 77.8\% of simulations, consistent with the $t$-test
    result. Together, these results indicate that both circumstantial
    factors fixed at draft time and controllable on-court performance
...
shows, the distribution is right-skewed (skewness $=1.11$). The two
\texttt{career\_length} definitions (span vs.\ distinct-season-count)
differ for \textbf{360 players} (those with a gap season); we use span
throughout for consistency with the paper.
...
test} (Fig.~\ref{fig:qq}) as the primary GOF result: $\chi^2=1480.6$,
$df=6$, $p<0.0001$ --- decisively rejecting normality. We additionally
fit a \textbf{Geometric distribution} by MLE ($\hat p=0.195$, implied
median survival $\approx3.2$ seasons) as a natural ``constant hazard''
model for career length, and test its fit with a second discrete
$\chi^2$ test (Fig.~\ref{fig:geom}): $\chi^2=601.9$, $df=7$,
$p<0.0001$ --- the geometric model is also rejected. This is itself an
informative finding: it implies the season-to-season exit probability
is \textit{not} constant over a career (e.g.\ likely higher in the
first 1--2 seasons and lower once a player is established), which
motivates the Kaplan--Meier / hazard-based approach discussed in
...
$\rho=-0.317$, $p<0.0001$. \textbf{Sanity check:} draft-age SD in the
incident-cohort sample is \textbf{2.40} seasons --- substantially
closer to the true NBA rookie-age SD of $\approx$1.5--1.7 than the
$\approx$3.4 implied by the pre-correction run, confirming the
incident-cohort filter meaningfully reduced left-truncation
contamination, though it may not have eliminated it entirely (see
Discussion). Notably, the corrected correlation ($r=-0.294$) is
\textit{stronger} than the pre-correction estimate ($r=-0.210$),
indicating the contamination had been \textit{diluting} the true
age-longevity relationship, not inflating it. Fig.~\ref{fig:h3} shows
the relationship (OLS slope $=-0.53$). \textbf{H3 is supported.}
...
largest-magnitude coefficient, but --- unlike the pre-correction run,
where it dwarfed every other predictor --- is now comparable to RPG,
consistent with the contamination being substantially resolved.
...
\subsection{H4: Nested Model Comparison --- Performance vs.\ Draft Round}
Following \citet{staw1995sunk} and \citet{groothuis2004exit}, we test
whether performance and draft round each carry independent
explanatory power by comparing three nested models
(Fig.~\ref{fig:nested}): \textbf{M1} (performance only, $R^2=0.203$),
\textbf{M2} (draft group only, $R^2=0.184$), and \textbf{M3} (both,
$R^2=0.251$). The nested $F$-test from M1 to M3 --- algebraically a
Generalized Likelihood Ratio Test for nested linear models
(Section~\ref{sec:methods}) --- tests whether draft group adds power
beyond performance: $F(2,2099)=66.7$, $p<0.0001$ (equivalently,
GLRT $-2\log\Lambda=130.0 \sim \chi^2_2$, $p<0.0001$). The reverse test
(M2 to M3) tests whether performance adds power beyond draft group:
$F(8,2099)=23.6$, $p<0.0001$. \textbf{Both directions are highly
significant}: performance and draft round each carry statistically
independent explanatory power, neither subsuming the other. This
directly confirms \textbf{H4} and replicates, on a modern
(1997--2022) cohort, the core finding of \citet{staw1995sunk} and
\citet{groothuis2004exit} that draft position affects career outcomes
even after controlling for on-court performance.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{fig17_nested_models.png}
  \caption{Nested model comparison: $R^2$ for performance-only,
...
A logistic regression on the same eight predictors, evaluated on a
held-out 25\% test set, achieved \textbf{63.8\% accuracy} and
\textbf{AUC$=0.693$} (Fig.~\ref{fig:roc}, confusion matrix
Fig.~\ref{fig:confusion}) --- meaningfully above the
\textbf{50.7\% majority-class baseline} (classes were nearly balanced:
50.7\% Long vs.\ 49.3\% Short). Coefficient signs match the regression:
PPG, RPG, APG, net rating, and height increase the odds of a long
career, while draft age decreases it.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.49\linewidth]{fig12_roc.png}\hfill
  \includegraphics[width=0.49\linewidth]{fig13_confusion.png}
  \caption{Long-vs-Short career classifier: ROC curve (left) and
...
5-fold cross-validation (Fig.~\ref{fig:cv}) gives mean CV $R^2=0.190$
($\pm0.023$), compared to in-sample $R^2=0.203$ --- a gap of only
$0.013$. This small gap indicates the regression model
\textbf{generalizes well} and is not meaningfully overfit.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{fig14_crossval.png}
  \caption{5-fold cross-validated $R^2$ vs.\ in-sample $R^2$.}
  \label{fig:cv}
\end{figure}

\subsection{Sequential Testing: Wald SPRT (1000-Simulation)}
We corrected a formula error in our original SPRT implementation (an
extraneous term had biased every log-likelihood-ratio increment toward
``Accept $H_0$'' regardless of the data) and now run the test across
1000 random re-orderings of the high-PPG subgroup rather than a single
pass, testing $H_0:\mu=5.13$ (overall mean) against
$H_1:\mu=6.44$ ($0.3\sigma$ above baseline; the subgroup's true mean,
6.39, lies close to $H_1$) (Fig.~\ref{fig:wald}). Across 1000
simulations, the corrected test \textbf{rejects $H_0$ (supports
$H_1$) in 77.8\%} of runs and accepts $H_0$ in 22.2\%, with no
``no-decision'' cases. The empirical mean stopping time (ASN) is
\textbf{48.1} observations (median 38), closely matching Wald's
theoretical ASN approximation of \textbf{42.4}. This is now consistent
with H1's much larger-sample $t$-test finding, unlike the single buggy
run in our earlier draft, which always accepted $H_0$ regardless of
the data due to the formula error.
...
Ratio Test (GLRT)}
To directly test H4 --- whether performance and draft round each carry
\textit{independent} explanatory power, rather than one subsuming the
other --- we compare three nested OLS models: performance-only (M1),
draft-group-only (M2), and both combined (M3). For nested linear
models under normally-distributed errors, the classical nested
$F$-test,
\[
...
improves the model beyond performance alone; testing M2$\to$M3 asks
the reverse.
...
subsumes the other. This is a more defensible conclusion than an
earlier draft's unadjusted comparison of two separate univariate gaps:
rather than asking which factor ``wins'' in isolation, H4 shows both
matter \textit{jointly}, replicating \citet{staw1995sunk} and
\citet{groothuis2004exit} on a modern cohort.
...
in-sample and cross-validated $R^2$ (0.203 vs.\ 0.190) indicates the
regression is not meaningfully overfit, and the classifier's 63.8\%
accuracy against a 50.7\% majority-class baseline (AUC$=0.693$)
confirms rookie-season variables carry genuine predictive signal.

\subsection{Two Corrections Made During Review}
Two issues found during methodological review meaningfully changed our
results.

\textbf{Left-truncation.} Taking each player's earliest \emph{recorded}
season as their rookie season silently misclassified players already
active before the dataset's 1996--97 start as young rookies, inflating
draft-age variance (implied SD $\approx$3.4 years pre-correction vs.\
the true NBA rookie-age SD of $\approx$1.5--1.7). This most directly
threatened H3 and the regression's draft-age coefficient. We corrected
it via incident-cohort filtering; the corrected draft-age SD is
\textbf{2.40} --- a substantial improvement, though still somewhat
above the textbook range (see Limitations).

\textbf{A formula error in the Wald SPRT.} Our original sequential-test
code used an incorrect log-likelihood-ratio increment (a spurious
constant term biased every step toward ``Accept $H_0$''), compounded
by running only a single sequence. We corrected the formula and now
report results across 1000 random re-orderings: the corrected test
rejects $H_0$ in 77.8\% of simulations, consistent with H1 ---
confirming the original ``Accept $H_0$'' result was a formula-bug
artifact, not a genuine statistical tension. Both corrections show
that sanity-checking implementation details against known real-world
quantities matters as much as choosing the
right test.
...
A third limitation is \textbf{residual left-truncation}: the corrected
draft-age SD (2.40) is closer to the true NBA rookie-age range
($\approx$1.5--1.7) than before, but not fully there --- our filter
only excludes players whose first recorded row is the dataset's first
season, missing players who took a multi-year gap before re-entering.
A stricter filter cross-referencing \texttt{draft\_year} could resolve
this fully. Finally, players are merged by \texttt{player\_name} (no
unique ID exists); we found no evidence of name-collisions, but note
it as a data-quality caveat.
...
length as final. This connects directly to our discrete goodness-of-fit
result: the geometric-distribution fit was decisively rejected
($\chi^2=601.9$, $p<0.0001$), implying the season-to-season exit
probability is \textit{not} constant over a career --- likely higher
in the first 1--2 seasons and lower once a player is established ---
which a Kaplan--Meier hazard curve could characterize directly. Beyond
this: a stricter incident-cohort filter using \texttt{draft\_year}
could resolve the residual left-truncation noted above, and the GLRT
comparison could extend to a three-way decomposition isolating
physical attributes separately.
